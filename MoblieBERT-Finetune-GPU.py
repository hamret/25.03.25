...
## 4. 학습용 및 검증용 데이터셋 분리 (scikit learn에 있는 train_test_split 함수 사용, random_state는 반드시 일치시킬것
...
# 5. moblieBERT 에 영화 리뷰 데이터를 Finetunning 하기 위한 데이터 설정
...
#6. 모델 생성
model = MobileBertForSequenceClassification.from_pretrained('mobliebert-uncased', num_labels=2)
model.to(device)
...
## 7. 학습(loss), 검증(train accurancy, validation accurancy)
epochs_results = []
...
for e in range(epochs):
    ...
    for batch in progress_bar:
        ...
        #학습 데이터 셋에 대한 평균 손실값 계산
    avg_train_loss = total_train_loss / len(train_dataloader)

    #학습 데이터 셋에 대한 정확도(accuracy) 계산
    model.eval()
    train_pred = []
    train_true = []
...
# 8. 학습 종료후 epoch 별 학습 경과 및 검증 정확도 출력
